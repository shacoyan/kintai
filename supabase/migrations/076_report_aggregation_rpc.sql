-- ============================================================================
-- 076_report_aggregation_rpc.sql
-- ----------------------------------------------------------------------------
-- 目的（Loop B / 日報・月報統合 — 集計 RPC 層）:
--   日報の「手入力データ」(public.daily_reports / 074) と月次固定マスタ
--   (public.store_monthly_settings / 075) を、Square（売上/客数/tender = Loop A
--   073 の RPC）と将来の人件費（Loop C）と合成して、日報・月報を集計表示する
--   ための public スキーマ RPC を 3 本作る。
--
--     (1) public.get_daily_report(store, date)        — 日報 1 日分
--     (2) public.get_monthly_report(store, year, month) — 店舗別 月次 P&L
--     (3) public.get_monthly_report_all(year, month)    — 7 店舗横断 総合 P&L
--
-- 対象 project:
...
--
-- 依存（適用順序・逆順厳禁）:
--   - 074(daily_reports) / 075(store_monthly_settings) が apply 済み。
--   - Loop A 073(tender breakdown) が apply 済み
--     = square_dashboard.get_sales_range_scoped / get_sales_by_location_scoped が
--       tender 4 キー（cash_amount/card_amount/external_amount/other_amount）を返す版。
--   - 既存ヘルパ（public.get_my_tenant_ids / is_tenant_managerial / is_my_store）。
--   - Loop C の public.parttime_labor_for_store(uuid,date,date) は無くても可
--     （to_regprocedure 動的判定で未実装時は 0・source='unavailable'）。
--
-- セキュリティ（最重要・設計 §4.1）:
--   - 3 本とも SECURITY DEFINER（RLS をバイパス）→ RPC が自前で
--     tenant_id IN get_my_tenant_ids() + (is_tenant_managerial OR is_my_store) を
--     WHERE で適用し、呼び出しユーザー権限内に絞る（fail-closed）。
--   - Square 側は square_dashboard.get_*_scoped を「修飾名」で呼ぶ。
--     これら自身が get_allowed_location_ids（072 SEC-1 込み）で auth.uid() ベースに
--     スコープ済 → 二重 fail-closed。
--   - search_path = public, pg_temp（square_dashboard は入れない＝必ず修飾名で呼ぶ）。
--
-- 全店一律の料率・税率（オーナー裁定3・2026-06-10）。変更はこの 3 定数のみ:
--   - 消費税: 内税10% → 売上総計 × 10/110
--   - カード決済手数料: 3.25% = 325 bps
--   - Paypay 決済手数料: 1.98% = 198 bps
--   ※ store_monthly_settings には料率列を持たない（075 で削除済）。
--
-- 経営数値の staff 非露出（裁定2帰結・設計 §4.6）:
--   get_monthly_report / _all は、対象 store の is_tenant_managerial でない場合
--   （= staff）、戻り jsonb の固定費生値（settings = 賃借料/社員給与/水道光熱費/
--   通信費/広告費/その他販管費）と target.sales_target を null にする。
--   派生指標（利益率 provisional_profit_rate / 達成率 achievement_rate）は返す。
--
-- インセンティブ統合（裁定1）:
--   月報「インセンティブ」= Σ daily_reports.incentive（バック金額と同一物）。
--   販管費【変動費】sga_variable に 1 回だけ含める（二重計上なし）。
--
-- 客数【総】= 新規+リピート+常連+スタッフ（4 セグ・unlisted 除外。Loop2 整合）。
-- 売上【総】= 決済済み total_amount Σ を基本（内訳 tender と整合）。open 込みは
--   total_with_open に併記（設計 §4.5 / 残オープン論点 e）。消費税・手数料の母数は
--   決済済みベース（total / card_amount / external_amount）。
--
-- store ↔ location 名称突合（設計 §1.6）:
--   kintai stores.name ↔ Square locations_meta.location_name は基本同名。
--   唯一の不一致 = 'こまいぬ'(kintai) ↔ '狛犬'(Square)。
--   - store → location: CASE WHEN name='こまいぬ' THEN '狛犬' ELSE name END
--   - location → store（_all の逆変換）: CASE WHEN location_name='狛犬'
--                                        THEN 'こまいぬ' ELSE location_name END
--
-- 金額型: kintai 手入力/固定費は integer（円）、Square は bigint。集計時は
--   numeric/bigint に昇格して計算し、戻りは jsonb（数値）で返す。
--
-- 冪等性: CREATE OR REPLACE FUNCTION + REVOKE（不在でも非エラー）で再実行安全。
-- 可逆性: 関数のみ（データ変更なし）。DROP FUNCTION で撤去可能。
-- ============================================================================


-- ============================================================================
-- (1) public.get_daily_report(p_store_id uuid, p_business_date date)
--     — 日報 1 日分（Square + daily_reports 手入力 + 人件費[Loop C] 合成）
-- ============================================================================
...
-- ============================================================================
-- (2) public.get_monthly_report(p_store_id uuid, p_year int, p_month int)
--     — 店舗別 月次 P&L
-- ============================================================================
...
-- ============================================================================
-- (3) public.get_monthly_report_all(p_year int, p_month int)
--     — 7 店舗横断 総合 P&L（呼び出しユーザー権限内の全店）
-- ============================================================================
...
BEGIN
  -- 権限内の全 store を列挙（managerial=全店 / staff=自店）。
  -- 各 store について get_monthly_report を呼び（権限・Square・手入力・人件費を一括処理）、
  ...
  FOR v_store_rec IN
    ...
    ORDER BY s.name
  LOOP
    v_one := public.get_monthly_report(v_store_rec.store_id, p_year, p_month);
...
-- ----------------------------------------------------------------------------
-- 権限テンプレ（MEMORY RLS 4 行・3 RPC すべて）。
--   REVOKE PUBLIC / REVOKE anon / GRANT authenticated。
-- ----------------------------------------------------------------------------
REVOKE EXECUTE ON FUNCTION public.get_daily_report(uuid, date)        FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_daily_report(uuid, date)        FROM anon;
GRANT  EXECUTE ON FUNCTION public.get_daily_report(uuid, date)        TO authenticated;

REVOKE EXECUTE ON FUNCTION public.get_monthly_report(uuid, int, int)  FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_monthly_report(uuid, int, int)  FROM anon;
GRANT  EXECUTE ON FUNCTION public.get_monthly_report(uuid, int, int)  TO authenticated;

REVOKE EXECUTE ON FUNCTION public.get_monthly_report_all(int, int)    FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_monthly_report_all(int, int)    FROM anon;
GRANT  EXECUTE ON FUNCTION public.get_monthly_report_all(int, int)    TO authenticated;
